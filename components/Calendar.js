...
const FEATURES = [
  {
    label: 'Navigate months',
    desc: 'Arrow buttons or swipe up/down on mobile',
  },
  {
    label: 'Country wise Calendar',
    desc: 'Pick your country for personalised public holidays and seasons',
  },
  { label: 'Click a day', desc: 'Select a date to open the Day panel' },
  {
    label: 'Mark / Unmark day',
    desc: 'Cross off a day; the cell dims with a red ✕',
  },
  { label: 'Emoji tags', desc: 'Add visual emoji labels to any day' },
  { label: 'Add tasks', desc: 'Type & press Enter — dot appears on the cell' },
  { label: 'Complete tasks', desc: 'Checkbox strikes through; ✕ deletes it' },
  { label: 'Month notes', desc: 'Free-text notes for the whole month' },
  {
    label: 'Download PDF',
    desc: 'Export day or month tasks as a printable PDF',
  },
]
...
const SEASON_THEMES = {
  winter: {
    ...
    '--c-accent-text': '#1e3a5f',
    '--page-bg': '#e4eaf5',
  },
  spring: {
    '--c-accent': '#16a34a',
    '--c-accent-dark': '#15803d',
    '--c-accent-light': '#dcfce7',
  ...
  summer: {
    ...
    '--c-accent-light': '#cffafe',
    '--c-accent-text': '#164e63',
    '--page-bg': '#ddf1f7',
  },
  autumn: {
    '--c-accent': '#d97706',
    ...
    '--c-accent-text': '#78350f',
    '--page-bg': '#f5ede0',
  },
}

function getSeason(month, country = 'IN') {
  const isSouthern = ['AU'].includes(country)
...
function playFlipSound() {
  try {
    new Audio('/audio/flip-sound.mp3').play()
  } catch {}
}
...
export default function Calendar() {
  ...
  const addTask = () => {
    ...
    setTasks((t) => ({
      ...t,
      [selectedKey]: [...(t[selectedKey] || []), task],
    }))
    setNewTask('')
  }
  const toggleTask = (id) =>
    setTasks((t) => ({
      ...t,
      [selectedKey]: (t[selectedKey] || []).map((tk) =>
  ...
  const deleteTask = (id) =>
    setTasks((t) => ({
      ...t,
      [selectedKey]: (t[selectedKey] || []).filter((tk) => tk.id !== id),
    }))
    ...
    if (mode === 'day' && selectedDay) {
      ...
      rows =
        Array.from({ length: days }, (_, i) => {
          ...
          .filter(Boolean)
          .join('') || '<p class="empty">No tasks this month.</p>'
    }
...
<script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()}<\/script></body></html>`
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(([e]) =>
      setCardWidth(Math.round(e.contentRect.width))
    )
    ro.observe(cardRef.current)
    return () => ro.disconnect()
  }, [])

  const season = getSeason(viewMonth, country)
        ...
        <div className={styles.welcomeOverlay}>
          <div className={styles.welcomeCard}>
            ...
            <div className={styles.welcomeBtns}>
              <button className={styles.welcomeSkip} onClick={handleSkipTour}>
                Skip tour
              </button>
              <button className={styles.welcomeStart} onClick={handleStartTour}>
                ▶ Start tour
              </button>
            </div>
          </div>
          ...
          <div className={styles.heroWrap}>
            ...
            {showInfo && (
              <div
                ...
                onClick={() => setShowInfo(false)} // click outside closes
              >
                <div
          ...
          <div className={styles.bottom}>
              ...
              {activeTab === 'day' && selectedDay && (
                <div className={styles.taskPanel}>
                  ...
                  <div id="cal-task-list" className={styles.taskList}>
                    ...
                    {dayTasks.length === 0 && (
                      <div className={styles.taskEmpty}>No tasks yet</div>
                    )}
                    {dayTasks.map((task) => (
                      <div key={task.id} className={styles.taskItem}>
                        ...
                        />
                        <span
                          className={`${styles.taskText} ${task.done ? styles.taskDone : ''}`}
                        >
                          {task.text}
                        </span>
                        <button
                  ...
                  <div id="cal-task-input" className={styles.taskInputRow}>
                    ...
                    />
                    <button className={styles.taskAddBtn} onClick={addTask}>
                      + Add task
                    </button>
                  </div>
                </div>
            ...
            <div className={styles.gridCol}>
              <div id="cal-month-nav" className={styles.monthNav}>
                ...
                <button className={styles.navArrow} onClick={nextMonth}>
                  ›
                </button>
              </div>
              ...
              <div className={styles.dayRow}>
                {DAYS.map((d) => (
                  <div
                    key={d}
                    className={`${styles.dayHead} ${d === 'SAT' || d === 'SUN' ? styles.weekend : ''}`}
                  >
                    {d}
                  </div>
                ))}
                ...
                {cells.map((c, i) => (
                  <button
                    ...
                    className={[
                      ...
                      c.weekend && c.valid && !c.isSelected
                        ? styles.cellWeekend
                        : '',
                  ...
                  >
                    ...
                    {c.emojis.length > 0 && (
                      <span className={styles.cellEmojiRow}>
                        {c.emojis.slice(0, 2).join('')}
                      </span>
                    )}
